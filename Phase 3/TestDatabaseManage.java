...
public class TestDatabaseManage {
    ...
    // Rule to create a temporary folder that will be used to store test files,
    // ensuring tests do not interfere with actual data.
    @Rule
    ...
    public void testCreateExistingAccount() throws AccountErrorException,
    ...
    public void testCreateAccountWithSpaceInPassword() throws AccountErrorException,
            PasswordErrorException {
    ...
    public void testCreateConversationSuccess() throws MessageSendErrorException {
        Conversation conversation = DatabaseManage.createConversation(sender, receiver);
        Assert.assertNotNull("Conversation should be successfully created", conversation);
        Assert.assertTrue("Message should be added to the conversation",
                conversation.getMessages().contains(message));
        Assert.assertTrue("Sender should have the conversation in their list",
                sender.getConversationFilenames().contains(conversation.getFilename()));
        Assert.assertTrue("Receiver should have the conversation in their list",
                receiver.getConversationFilenames().contains(conversation.getFilename()));
    }
    ...
    public void testCreateConversationBlockedSender() throws MessageSendErrorException {
        ...
        DatabaseManage.createConversation(sender, receiver);
    }
