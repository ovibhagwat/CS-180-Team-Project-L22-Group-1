...
public class User implements UserInterface {
    ...
    public boolean isFriend(String user) { return getFriendsList().contains(user); };
    @Override
    public ArrayList<String> getFriendsList() {
        return friendsList;
    }
    public void addFriend(String accountID) throws FriendBlockErrorException {
        if (getFriendsList().contains(accountID)) {
            throw new FriendBlockErrorException("You are already friends with this user!");
        } else {
            getFriendsList().add(accountID);
        }
        writeUser(); // Save changes to user data file.
    }
    public void removeFriend(String accountID) throws FriendBlockErrorException {
        if (!getFriendsList().contains(accountID)) {
            throw new FriendBlockErrorException("You are not friends with this user.");
        } else {
            getFriendsList().remove(accountID);
        }
        writeUser(); // Save changes to user data file.
    }
    ...
    public void addBlock(String accountID) throws FriendBlockErrorException {
        if (getBlockList().contains(accountID)) {
            throw new FriendBlockErrorException("You already have this user blocked!");
        } else {
            getBlockList().add(accountID);
        }
        writeUser(); // Save changes to user data file.
    }
    public void removeBlock(String accountID) throws FriendBlockErrorException {
        if (!getBlockList().contains(accountID)) {
            throw new FriendBlockErrorException("You do not have this user blocked.");
        } else {
            getBlockList().remove(accountID);
        }
        writeUser(); // Save changes to user data file.
    }
