...
public class TestMessage {
    ...
    public void testSerializationConstructor() {
        ...
        Assert.assertTrue("Checking message timestamp from serialized data", Math.abs(currentTime - message.getTimestamp().getTime()) < 1000);
    }
    ...
    public void testToStringMethod() {
        ...
        String content = "Hello, Bob!";
        Message message = new Message(type, sender, receiver, content);
        String expectedString = type + "|" + message.getTimestamp().getTime() + "|" + sender + "|" + receiver + "|" + content;

        String toStringOutput = message.toString();
        // Verify the toString output matches the expected format
        Assert.assertTrue("The toString method should contain the type", toStringOutput.startsWith(type + "|"));
        Assert.assertTrue("The toString method should contain the sender", toStringOutput.contains("|" + sender + "|"));
        Assert.assertTrue("The toString method should contain the receiver", toStringOutput.contains("|" + receiver + "|"));
        Assert.assertTrue("The toString method should end with the content", toStringOutput.endsWith("|" + content));
    }
